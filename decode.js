...
message.addEventListener("input", function handleChange(event) {
	...
	for (let i = 0; i < string_split_sentence.length; i++) {
		word_split_var_array[i] = string_split_sentence[i].split(split_by_no);
	}
	...
	for (let i = 0; i < word_split_var_array.length; i++) {
		for (let j = 0; j < word_split_var_array[i].length; j++) {
			words_letters_index[i][j] = new_alph_array.indexOf(
	...
	for (let i = 0; i < string_split_sentence.length; i++) {
		for (let j = 0; j < word_split_var_array[i].length; j++) {
			new_word_letters_array[i][j] = alph_array[words_letters_index[i][j]];
		}
	}
	//change words
	var change_words_array = [];
	for (let i = 0; i < string_split_sentence.length; i++) {
		change_words_array[i] = new_word_letters_array[i].join(join_mth_ltr);
	}
	//change sentence
